...
对一组交易 S 重新排序时遇到的挑战：首先我们需要确定 S 中的哪些交易是相互冲突的。由于 S 中的交易在完全隔离的环境下模拟执行，这些冲突仍有可能会发生。因为S 的 提交发生在之后的阶段，它们没有机会看到彼此潜在的冲突修改。
...
首先从 C(S’) 中下标最小的交易 T1 开始（即代码中的 startNode），我们必须找到 T1 的源节点，将它排在最后。T1 有两个父节点 T3 和 T4，T3 没有被访问过但是它有父节点 T4，T4没有被访问过且是源节点，因此我们将 T4 放在 schedule 的最后一个位置。
