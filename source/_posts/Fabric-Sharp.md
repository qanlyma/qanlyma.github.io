...
* 通过将 FabricSharp 与 Fabric、Fabric++ 和其他技术进行比较。实验结果表明，与其他系统相比，FabricSharp 的吞吐量高出 25% 以上。
...
* **Definition 3.2 (Snapshot consistency)**. 如果存在从中读取所有交易记录的区块链快照 M，则交易是快照一致的。
...
* **Proof**. 首先证明任何没有 anti-rw 交易的 schedule 实现了可序列化：通过反证法，假设一个 schedule 不能实现可序列化，那么其中必然有一个交易子集具有环形依赖，其中最后提交的交易由 Txn 表示。那么 Txn 必须具有 anti-rw 依赖关系，因为 anti-rw 是唯一将后面的与前面的交易关联起来的依赖关系，这与假设不符。因此 schedule 实现了可序列化。下一步证明实现了强可序列化：由于其余五个依赖项的顺序与其提交顺序一致，因此遵循提交顺序的序列化历史也遵循依赖顺序。根据冲突可串行化定理，这种序列化的交易历史具有可序列化 schedule 的等效效果。因此，schedule 是强序列化的。
...
* **Theorem 3.16**. 如果存在一个没有涉及 c-ww 依赖关系的循环，则 schedule 无法重新排序为可序列化。
...
然而，如果在待定的交易之间存在一个具有 c-ww 冲突的循环，则 schedule 可以被重新排序为可序列化。由于 Lemma 3.15，它们的依赖顺序可以翻转。图 b 中展示了这种场景，其中由 Txn1、Txn2 和 Txn3 形成的循环调度通过切换 Txn2 和 Txn3 的提交顺序而变得可串行化，这表现出 c-ww 依赖性。

### 3.4 Fine-grained Concurrency Control

Theorem 3.16指出，在待定的交易中没有 c-ww 的循环 schedule，即使重新排序也无法序列化。基于此，我们为 EOV 区块链中的细粒度并发控制制定了以下三个步骤：
...
我们的重新排序算法是排序过程的一部分，需要在共识建立交易顺序后，在每个诚实的排序节点复制以形成账本。假设原始共识服务在其安全模型下实现了安全性和有效性，现在讨论在重新排序后这两个性质是否保持不变。
...
为了计算算法 2 中的依赖图，我们在排序节点中引入了两个多版本存储，以识别已提交的交易。这些存储器在 LevelDB 中实现，分别表示为 CommittedWriteTxns（CW）和 CommittedReadTxns（CR）。CW 中的键值由记录键值和更新值的交易的提交序列组成。例如，Txn1 的提交序号是 (3,2) 写入 A 的值，CW 会记录 {A_3_2:Txn1}。CR 中的键值由记录键值和读取最新值的交易的提交序列组成。。

我们在内存中维护两个索引，PendingWriteTxns（PW）和 PendingReadTxns（PR），分别存储待定交易的写入和读取集。如果一个新交易 txn 开始于 start，读取 R，写入 W，那么 txn 的所有依赖计算如下：

![Dependencies Computing](13.png)

请注意，我们忽略待定交易之间的 ww 依赖关系，并且不区分 ww 和 rw 是否并发。这是因为非并发交易可能是一个环的一部分。然后将 txn 的之前交易计算为 ww(txn) ∪ n-wr(txn) ∪ rw(txn)，将后继交易计算为 anti-rw(txn)。

### 4.4 Cycle Detection

本节讨论如何使用依赖图 G 来检测环并实现可序列化。有两个设计选择：一方面我们可以只维护每个交易的即时链接信息，然后执行循环检测的图遍历；另一方面我们可以维护每对交易之间的整个可达性信息。后者将开销从计算转移到空间消耗。我们通过维护交易的直接继承者 (txn.succ) 来实现最佳点，并用布隆过滤器表示所有可以到达 txn 的交易，称为 txn.anti_reachable。循环检测可以直接通过对由 txn 的前一个和后一个组成的每对 (p,s) 测试 p.anti_reachable(s)。

![Algorithm 4](14.png)

然而，已知布隆过滤器会报告 false positives。如果过滤器向 txn 报告了一对相邻交易的 false positives，我们会预防性地中止 txn。如果报告所有交易对都是 negative，那么 txn 不属于 G 中的任何环。
