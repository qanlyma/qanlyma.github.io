...
```bash
# 将文件 file.txt 中的 "old_text" 替换为 "new_text" 并输出结果到标准输出
sed 's/old_text/new_text/' file.txt

# 替换文件 file.txt 中所有匹配的 "old_text" 为 "new_text"，并将结果写回到原文件
...
但过度使用索引也可能会导致一些问题。索引需要占用额外的存储空间，并且在数据插入、更新和删除时需要额外的维护成本。
...
* B+ 树的磁盘读写代价更低：B+ 树的内部节点并没有指向关键字具体信息的指针，因此其内部节点相对 B 树更小，如果把所有同一内部节点的关键字存放在同一盘块中，那么盘块所能容纳的关键字数量也越多，一次性读入内存的需要查找的关键字也就越多，相对 IO 读写次数就降低了。B 树中每个节点中不仅包含数据的 key 值，还有 data 值。而每一个页的存储空间是有限的，如果 data 数据较大时将会导致每个节点（即一个页）能存储的 key 的数量很小，当存储的数据量很大时同样会导致 B 树的深度较大，增大查询时的磁盘 I/O 次数，进而影响查询效率。
