...
1. 命名空间（Namespaces）：命名空间是 Linux 内核中的一种机制，允许容器拥有独立的资源视图。Docker 使用了多个命名空间，如进程命名空间、网络命名空间、挂载命名空间等，以确保每个容器看到的资源都是隔离的，不会与其他容器产生冲突。
2. 控制组（Cgroups）：控制组是 Linux 内核的另一个特性，允许限制和隔离资源的使用，如 CPU、内存、网络带宽等。Docker 使用 Cgroups 来确保容器使用的资源受到限制，防止一个容器占用全部系统资源导致其他容器受影响。
3. 文件系统：Docker 使用 UnionFS（联合文件系统）来实现容器的文件系统隔离。每个容器都有自己的文件系统层，并且可以共享基础镜像的文件层，从而节省磁盘空间。这样的设计允许容器拥有独立的文件系统视图，互相之间不会干扰。
4. 网络隔离：Docker 为每个容器创建独立的网络命名空间，每个容器都有自己的 IP 地址、网络接口和端口，从而实现容器之间的网络隔离。
5. 用户隔离：Docker 允许在容器内部使用不同的用户和用户组，从而限制容器内的权限，使其无法访问宿主系统的敏感资源。

## 数据库

#### Q：索引

数据库索引是数据库管理系统中一个排序的数据结构，以协助快速查询、更新数据库表中数据。如果没有索引，数据库引擎需要扫描整个数据表来找到匹配的记录。而有了索引，数据库引擎可以直接跳到索引中存储的位置，快速定位到匹配的记录，从而大大加快查询速度。
