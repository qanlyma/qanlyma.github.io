...
可以通过共享内存（变量）加锁的方式来进⾏通信，但是维护成本较⾼。官⽅推荐通过 channel 进⾏通信：
...
3. 安全性：Cookie 存储在客户端，可以被客户端修改或者删除。Session 数据存储在服务器端，相对来说更安全，但是会占用服务器资源。
...
* 分页是将进程的逻辑地址空间划分成大小固定的页（Page），同时将物理内存划分成与页大小相同的块，称为页框（Page Frame）。每个进程的页表（Page Table）记录了逻辑页到物理页框的映射关系。通过分页，进程的逻辑地址空间可以被分割成大小相等的部分，而物理内存则被划分成相同大小的页框，这样使得进程的内存管理更加灵活，且易于实现虚拟内存和内存保护。
* 分段是将进程的逻辑地址空间划分成多个逻辑段（Segment），每个段表示一个独立的逻辑单元，如代码段、数据段、栈段等。每个段的大小可以不同，并且段的长度可以根据进程的需求动态增长或缩小。分段的主要目的是提供更为灵活的内存管理，适用于变长数据结构和程序。
...
* 在 Redis中，没有像传统数据库中的索引那样的结构。所有的数据都存储在内存中，因此数据的访问速度非常快，不需要像传统数据库一样通过索引来查找。

#### Q：为什么要用 Redis

性能：将频繁使用的数据存在缓存中，再访问这些数据的时候就可以直接从缓存中获取，速度相当快。

并发：在大并发的情况下，所有的请求直接访问数据库，数据库会出现连接异常。可以使用 Redis 做一个缓冲。

#### Q：单进程单线程的 Redis 为什么这么快

单线程指的是网络请求模块使用了一个线程（所以不需考虑并发安全性）处理所有网络请求，其他模块仍用了多个线程。
* 绝大部分请求是纯粹的内存操作
...
* AOF：Append Only File，每执行一条更改 Redis 中的数据的命令，Redis 就会将该命令写入硬盘中的 AOF 文件。
