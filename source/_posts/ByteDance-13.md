...
可以通过共享内存（变量）加锁的方式来进行通信，但是维护成本较⾼。官⽅推荐通过 channel 进行通信：
...
* 分页是将进程的逻辑地址空间划分成大小固定的页（Page），同时将物理内存划分成与页大小相同的块，称为页框（Page Frame），每个进程的页表（Page Table）记录了逻辑页到物理页框的映射关系。分页消除了外部碎片，但会出现内部碎片。
...
运行的不正常，或者不稳定，要花费更多的资需源处理别的事情，会使 SQL 语句的执行效率明显变慢。

针对 innoDB 导致 MySQL 抖的原因，主要是 InnoDB 会在后台刷脏页（内存数据页跟磁盘数据页内容不⼀致），而刷脏页的过程是要将内存页写⼊磁盘。所以，⽆论是查询语句在需要内存的时候可能要求淘汰⼀个脏页，还是由于刷脏页的逻辑会占用 IO 资源并可能影响到了更新语句，都可能是造成业务端感知 MySQL 抖了⼀下的原因。
