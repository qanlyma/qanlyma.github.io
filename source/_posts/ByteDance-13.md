...
* sync.Map：Go 语言提供了一些第三方库实现了并发安全的 Map，可以在多个 Goroutine 中并发读写，无需额外的锁保护。
...
可以通过共享内存（变量）加锁的⽅式来进⾏通信，但是维护成本较⾼。官⽅推荐通过 channel 进⾏通信：

* 作为队列存储数据
* 阻塞和唤醒 goroutine

select 搭配 channel 使⽤，其机制是监听多个 channel，每⼀个 case 都是⼀个事件，⼀旦某个事件就绪（chan 没有堵塞），就会从这些就绪事件中随机选择⼀个去执⾏，default ⽤于所有 chan 都堵塞的情况执⾏。
...
3. ⼿动触发 GC：可以在代码中通过使⽤ runtime.GC() 来⼿动触发
...
Go 中的变量只有在编译器可以证明在函数返回后不会再被引用的，才分配到栈上，其他情况下都是分配到堆上。Go 语言里没有一个关键字或者函数可以直接让变量被编译器分配到堆上，相反，编译器通过分析代码来决定将变量分配到何处：
...
* 超时重传
* 连接管理
* 流量管理：滑动窗口机制基于接收方的反馈信息来动态调整发送方的发送速率
...
* IPv4 是目前广泛使用的 IP 协议版本。它使用 32 位地址，通常表示为**点分十进制**格式（例如，192.168.0.1）。
...
* 线程（Thread）：线程是**进程内的执行单元**，是 **CPU 调度的最小单位**。一个进程可以包含多个线程，共享相同的内存空间和上下文环境。不同线程之间可以并发执行，通过共享内存来进行数据通信，但也需要注意同步和互斥问题。每个线程都有自己独立的堆栈空间，线程之间的切换开销相对较小，适合执行密集的计算和并发任务。

* 协程（Coroutine）：协程是一种**用户态的轻量级线程**，一个线程中可以包含多个协程，在同一时刻只有一个协程运行，通过协程的切换来实现并发任务的处理。协程在执行过程中可以主动挂起和恢复，控制权的切换由程序自身控制，通常不涉及内核态的切换，因此协程的创建和切换开销较小。协程通常使用更少的内存，因为多个协程可以共享同一个线程的堆栈空间。
...
* 并发是一种任务调度的概念，表示多个任务在同一时间段内交替执行
* 并行是一种任务执行的概念，表示多个任务在同一时刻同时执行
...
分页和分段是计算机内存管理中的两种重要技术，它们的主要作用是将进程的逻辑地址空间映射到物理内存地址，实现进程的隔离和保护，并提供了更好的内存管理和利用。

* 分页是将内存空间划分为固定大小的页（Page），进程的逻辑地址由两部分组成：页号和页内偏移量。通过页表（Page Table）来记录页号和页框之间的映射关系，将进程的页号转换为物理内存的页框号，再加上页内偏移量得到实际的物理地址。分页的优点是：简单、灵活、易于管理和实现。它可以允许进程的逻辑地址空间非连续，提供了更好的内存利用率。但由于地址映射需要额外的页表查询，会引入一定的开销。
* 分段是内存空间划分为不同大小的段（Segment），每个段用于存放特定类型的数据或代码。进程的逻辑地址由两部分组成：段号和段内偏移量。通过段表将段号转换为段的起始地址，再加上段内偏移量得到实际的物理地址。分段的优点是：能够适应动态变化的进程地址空间，方便实现动态内存分配和共享。但由于每个段有不同的大小，可能会导致内存碎片问题，并且段表查询也会引入一定的开销。
...
Kubernetes 是一个用于自动化部署、扩展和管理容器化应用程序的容器编排平台。它的目标是处理大规模、高可用性的分布式系统。

k8s 管理着每个容器的全生命周期，如根据需要启动或停止容器。通过自动布局、自动重启、自动复制、自动伸缩实现应用的状态检查与自我修复。使用命令、UI 或者基于 CPU 使用情况自动快速扩容和缩容应用程序实例，保证应用业务高峰并发时的高可用性；业务低峰时回收资源，以最小成本运行服务。为多个容器提供服务发现和负载均衡，使得用户无需考虑容器 IP 问题。
...
Master 节点上有 API server、controller-manager、scheduler 以及使用 Etcd 做 k8s 集群存储；
Slave 节点上有 kubelet、kube-proxy、容器引擎（比如 docker），每个 Slave 节点都会被 Master 分配一些工作负载。

* API server 负责接收 k8s 所有请求，所有对象资源的增删改查和监听操作都交给 API Server 处理后再提交给 Etcd 存储。
...
但索引需要占用额外的存储空间，并且在数据插入、更新和删除时需要额外的维护成本。

关系型数据库索引的实现通常使用 B 树及其变种 B+ 树。
...
* B+ 树的数据都存储在叶子结点中方便扫库，只需要扫一遍叶子结点即可，但是 B 树因为其分支结点同样存储着数据，需要进行一次中序遍历按序来扫，所以 B+ 树更加适合在区间查询的情况。
...
聚簇索引直接将表的数据行存储在索引结构中，而非聚簇索引则将索引和数据行分开存储。优点是查询速度快，缺点是数据更新代价高。
...
2. Repeatable read (可重复读)：
   * 可避免脏读、不可重复读的发生。
   * 多次读取同一数据，确保在事务执行期间，同一数据的值保持不变。
3. Read committed (读已提交、不可重复读)：
   * 可避免脏读的发生。
   * 只能读取到已经提交的数据，但不能读取未提交的数据。
...
#### Q：MySQL 存储引擎 MyISAM 与 InnoDB 区别

1. 事务支持：
  MyISAM：不支持事务。
  InnoDB：实现了 ACID 属性，可以进行事务的提交和回滚。

1. 行级锁：
  MyISAM：采用表级锁，意味着在执行写操作时，会锁定整个表，这可能导致并发写操作之间的冲突和阻塞。
  InnoDB：采用行级锁，它可以在并发情况下实现更好的性能和并发度，不同行之间的写操作不会相互影响。

1. 外键约束：
  MyISAM：不支持外键约束，无法通过外键关联来保持数据的完整性。
  InnoDB：支持外键约束，它可以保持数据的完整性，并允许在表之间建立关联。

1. 表的关系：
  MyISAM：适合用于读密集型应用，因为它对于大量的 SELECT 查询性能较好。
  InnoDB：适合用于读写混合型应用，因为它支持事务和行级锁，并且在并发写入情况下具有更好的性能。

1. 崩溃恢复：
  MyISAM：在崩溃后的恢复速度较快，但可能会有数据丢失。
  InnoDB：在崩溃后的恢复速度较慢，但通常不会有数据丢失，因为它支持事务和日志记录。
...
* 在 Redis中，没有像传统数据库中的索引那样的结构。所有的数据都存储在内存中，因此数据的访问速度非常快。当你需要访问某个键对应的值时，只需要直接通过键名来获取，不需要像传统数据库一样通过索引来查找。
...
主从复制是 Redis 的一个重要功能，它可以使得一个 Redis 服务器（Master）的数据自动地被复制到其他 Redis 服务器（Slave），从而实现数据的备份、容灾和读写分离。

* Master（主节点）：
  * 主节点是 Redis 复制的源头，它包含着完整的数据集。
  * 主节点接收客户端的写请求，对数据进行修改，并将修改后的数据同步给所有从节点。
  * 主节点可以有多个从节点连接到它，并且可以同时服务于读写请求。

* Slave（从节点）：
  * 从节点是主节点的复制品，它复制主节点的所有数据。
  * 从节点只接收来自主节点的数据同步，不能处理客户端的写请求。
  * 从节点可以用于读操作，从而实现读写分离，减轻主节点的读压力。
  * 从节点也可以用于实现高可用性和故障转移。当主节点不可用时，从节点可以被提升为主节点，继续服务于客户端的读写请求。
...
并发数、响应时间、吞吐量、资源利用率（CPU、内存、带宽）、错误率、响应时间分布、瓶颈分析、平台和网络性能。
